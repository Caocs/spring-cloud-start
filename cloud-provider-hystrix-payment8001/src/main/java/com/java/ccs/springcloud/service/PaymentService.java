...
/**
 * @Author: Cao.cs
 * @Date: Created in 20:19 2022/3/23
 */
@Service
public class PaymentService {
    ...
    /**
     * 测试服务降级：
     * （1）超时访问，设置自身调用超时的峰值，峰值内正常运行，超过了峰值需要服务降级，自动调用fallbackMethod指定的方法
     * （2）超时异常或者运行异常，都会进行服务降级
     * <p>
     * fallbackMethod = "paymentInfoTimeOutHandler" ：触发降级时，对应的执行方法。
     *
     * @HystrixProperty(name = "execution.isolation.thread.timeoutInMilliseconds", value = "5000") 方法执行超时时间，如果超时则触发降级
     */
    @HystrixCommand(
    ...
    /**
     * paymentInfoTimeOut 方法失败后，自动调用此方法，实现服务降级
     * 告知调用者 paymentInfoTimeOut 目前无法正常调用。
     */
